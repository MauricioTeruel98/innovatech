...
$unreadStmt = $db->prepare("SELECT COUNT(*) FROM contact_messages WHERE is_read = 0 AND site = ?");
$unreadStmt->execute([$site]);
$unreadMessages = (int) $unreadStmt->fetchColumn();

if ($site === 'labs') {
    $blocks  = (int) $db->query("SELECT COUNT(*) FROM lab_blocks WHERE site='labs'")->fetchColumn();
    $plans   = (int) $db->query("SELECT COUNT(*) FROM lab_plans WHERE site='labs'")->fetchColumn();
    $tCount  = (int) $db->query("SELECT COUNT(*) FROM testimonials WHERE site='labs'")->fetchColumn();
    $stats = [
        ['icon' => 'bi-boxes',       'value' => $blocks, 'label' => 'Bloques de contenido'],
        ['icon' => 'bi-tags',        'value' => $plans,  'label' => 'Planes'],
        ['icon' => 'bi-chat-quote',  'value' => $tCount, 'label' => 'Testimonios'],
    ];
} else {
    ...
    $stats = [
        ['icon' => 'bi-mortarboard-fill', 'value' => $totalCourses,   'label' => 'Cursos totales'],
        ['icon' => 'bi-star-fill',        'value' => $popularCourses, 'label' => 'Cursos destacados'],
        ['icon' => 'bi-people-fill',      'value' => $teamCount,      'label' => 'Miembros del equipo'],
    ];
}
...
<p style="color:#8b8bae; font-size:0.9rem; margin:-6px 0 18px">
    Administrando <strong style="color:#4db8a0"><?= htmlspecialchars($siteInfo['name']) ?></strong>.
    Usá el switch de arriba a la derecha para cambiar de sitio.
</p>
...
<div class="data-table">
    <table class="table table-borderless mb-0">
        <thead>
            <tr><th>Título</th><th>Categoría</th><th>Nivel</th><th>Última actualización</th><th></th></tr>
        </thead>
        <tbody>
            ...
            <tr>
                ...
                <td><?php $lvl = strtolower($c['level']); ?><span class="badge-level badge-<?= $lvl ?>"><?= htmlspecialchars($c['level']) ?></span></td>
                <td style="color:#8b8bae; font-size:0.8rem"><?= date('d/m/Y H:i', strtotime($c['updated_at'])) ?></td>
                <td><a href="<?= PANEL_URL ?>/courses/edit.php?id=<?= $c['id'] ?>" class="btn btn-sm btn-outline-light py-1 px-3"><i class="bi bi-pencil"></i></a></td>
            </tr>
            <?php endforeach; ?>
        </tbody>
    </table>
